 ...
 if     (!((CheckPermission('Plugins.MembersListEnh.GenView')) ||
   ...
   if (!is_numeric($Limit) || $Limit < 0)
    ...
    case 'liked':
            $UserField  = 'Liked';
           break;  
     case 'countacceptedanswers':
            $UserField = 'CountAcceptedAnswers';
            break;
...
<table id="Users" class="AltColumns" style="width: 100%;">
   <thead>
      ...
      if ($ShowSymbol)
       ...
       if ($ShowAnswers)
        echo '<th>' .Anchor(T('Answers'),$MColUrl . '&ufield=' . 'CountAcceptedAnswers' . '&sort=' . $NewSort ) . '</th>';      
        if ($ShowID)
     ...
     <tr<?php echo $Alt ? ' class="Alt"' : ''; ?>>
      <?php  
        ...
        if ($ShowKarma) { 
         ...
         if ($ShowAnswers)
         echo '<td>' . $User->CountAcceptedAnswers. '</td>';    
